...
class Calendar {
  ...
  get(hash) {
    ...
    let section = this.dataLib.getSectionServerDataFromHash(hash)
  ...
  for (let meeting of section.meetings) {
      let startDate = moment((serverData.startDate + 1) * 24 * 60 * 60 * 1000);
      let endDate = moment((serverData.endDate + 1) * 24 * 60 * 60 * 1000);
