...
Macros.parseNameWithSpaces = function parseNameWithSpaces(name) {
  // Standardize spaces.
...
Macros.standardizeEmail = function standardizeEmail(email) {
...
Macros.standardizePhone = function standardizePhone(phone) {
  phone = phone.trim();
...
Macros.parseGoogleScolarLink = function parseGoogleScolarLink(link) {
  if (!link) {
    return null;
  }
...
Macros.getBaseHost = function getBaseHost(url) {
...
Macros.occurrences = function occurrences(string, subString, allowOverlapping) {
  string += '';
  subString += '';
...
Macros.loadDevData = async function loadDevData(path) {
  if (!Macros.DEV) {
...
Macros.saveDevData = async function saveDevData(path, data) {
  if (!Macros.DEV) {
...
Macros.critical = function critical(...args) {
...
Macros.error = function error(...args) {
...
Macros.verbose = function verbose(...args) {
