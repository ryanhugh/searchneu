...
function PageData(startingData) {
  ...
  for (const attrName in startingData.dbData) {
...
PageData.create = function (startingData) {
  const pageData = new PageData(startingData);
  if (!pageData.dbData) {
...
PageData.prototype.findSupportingParser = function (parserName) {
  ...
  const parsers = pageDataMgr.getParsers();
...
PageData.prototype.processDeps = function (callback) {
  ...
  asyncjs.map(this.deps, (depPageData, callback) => {
    pageDataMgr.processPageData(depPageData, (err, newDepData) => {
      if (err) {
...
PageData.prototype.getUrlStart = function () {
  const urlParsed = new URI(this.dbData.url);
...
PageData.prototype.addDep = function (depData) {
  ...
  const valuesToCopy = ['host', 'termId', 'subject', 'classId', 'crn'];
  valuesToCopy.forEach((attrName) => {
  ...
  for (let i = 0; i < this.deps.length; i++) {
    ...
    if (isMatch) {
      ...
      for (const newAttrName in depData) {
  ...
  const dep = this.constructor.create(startingData);
  if (!dep) {
